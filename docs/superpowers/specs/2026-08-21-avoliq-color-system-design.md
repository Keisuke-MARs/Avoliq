...
- `window-vibrancy` による Liquid Glass の実装
- 選択カード・ステータスチップの表現変更
...
合成モデル（`HudWindow` を ライト≒白 0.72 相当 / ダーク≒黒 0.78 相当 と近似）での実効背景:
...
**既知の欠陥の同時修正**: `StatusSettings.tsx` の現在色の逆引きが `-1` を返したとき、
`STATUS_COLORS[-1]?.value` が `undefined` になり変更が黙って破棄される経路がある。
プリセット外の色（既存ボードに残る `#007AFF` など）で実際に起きるため、`-1` の場合は 0 番にフォールバックする。
...
- **`StatusSettings` の `-1` フォールバック**: プリセット外の色（既存ボードに残る `#007AFF`）を持つステータスで
  色変更が破棄されないことを回帰テストで固定する。
